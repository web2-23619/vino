...
(async function () {
    ...
    // Event listener for the "Ajouter au cellier" button
    document.addEventListener("click", (event) => {
        if (event.target.matches('[data-js-action="addToCellar"]')) {
            ...
            let source = 'cellier'; // Default to 'cellier'
            if (window.location.href.includes("listeAchat")) {
                source = 'listeAchat';
            }
    
            // Redirect to the form with the correct source
            window.location.href = `/cellier/bouteille/ajouter/${bottleId}?source=${source}`;
        }
    });
